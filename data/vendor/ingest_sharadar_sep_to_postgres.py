#!/usr/bin/env python3
"""
Ingest SHARADAR/SEP (equity daily prices) into Postgres.

SHARADAR/SEP provides daily OHLCV + adjusted close for US equities.
...
TABLE_CODE = "SHARADAR/SEP"
SOURCE_TABLE = "SEP"

# Columns the API must return (rule 2c) and the full known set (rule 2b).
REQUIRED_SEP_COLS: set[str] = {"ticker", "date", "close"}
KNOWN_SEP_COLS: set[str] = {
...
def get_max_trade_date(conn: psycopg.Connection, schema: str) -> str | None:
    """Return the most-recent trade_date for SEP rows in daily_bars, or None if empty."""
    with conn.cursor() as cur:
        cur.execute(
...
def upsert_instruments_from_bars(conn: psycopg.Connection, schema: str) -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT DISTINCT s.symbol, 'STOCK', '{TABLE_CODE}', TRUE, NOW()
            FROM stg_bars s
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest SHARADAR/SEP into Postgres")
...
def main(argv: list[str]) -> int:
    ...
    with psycopg.connect(config.db_dsn) as conn:
        ...
        validate_sample_shape(TABLE_CODE, sample_cols, sample_rows, REQUIRED_SEP_COLS)

        # ── STEP 2: schema-drift check (rule 2b) ──────────────────────────────
        drift = detect_schema_drift(TABLE_CODE, set(sample_cols), KNOWN_SEP_COLS)
        max_date = get_max_trade_date(conn, config.schema)

        if config.overwrite or max_date is None:
            reason = "--overwrite" if config.overwrite else "table is empty (no SEP rows)"
